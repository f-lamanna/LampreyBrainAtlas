#setwd("~/LampreyBrainApps/Apps/am_brain/")
source(file = "prepare_env.R")

# User interface (client)
ui <- navbarPage(title = "LarvalEncephalon", 
                 id = "lb",
                 position = "fixed-top",
                 theme = "bootstrap_am_brain.css",
                 ...
                 tabPanel(title = "Taxonomy",
                          ...
                          fluidRow(column(width = 4,
                                          offset = 2,
                                          plotlyOutput(outputId = "seurat_umap_2d") %>% withSpinner(color= "#C0C0C0")),
                                   column(width = 4,
                                          plotlyOutput(outputId = "seurat_tsne_2d") %>% withSpinner(color= "#C0C0C0")))
